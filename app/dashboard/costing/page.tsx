...
export default function CostingDashboard() {
  // Plan Tier State (basic, pro, yearly)
  ...
  const handleSaveCalculation = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        alert(data.error || "Save limit reached. Upgrade to Pro for unlimited saves.");
        return;
      }
  ...
  const handleAddComponent = () => {
    if (isBasic) {
      alert(
        "🔒 Feature Locked: Adding custom components & extra rows/columns is available only on the Pro (₹599) and Yearly plans. Please upgrade."
      );
      return;
    }
    ...
    <main className="min-h-screen bg-slate-950 text-slate-100 p-6 md:p-10 selection:bg-emerald-500 selection:text-black">
      <div className="max-w-7xl mx-auto space-y-8">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className={`text-[11px] font-bold px-2.5 py-0.5 rounded-full uppercase tracking-wider ${
                isBasic ? "bg-amber-950 text-amber-400 border border-amber-500/30" : "bg-emerald-950 text-emerald-400 border border-emerald-500/30"
              }`}>
                {userTier} Plan
              </span>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {/* Main Calculation Column (2 Cols) */}
                ...
                {isBasic ? (
                  <div className="p-4 border-2 border-dashed border-slate-800 rounded-xl bg-slate-950/50 text-center">
                    ...
                    <p className="text-[11px] text-slate-500">
                      Upgrade to <Link href="/pricing" className="text-emerald-400 underline font-semibold">Pro (₹599)</Link> to upload garment sketches & tech packs.
                    </p>
                  </div>
                ) : (
                  <input
              ...
              <div className="flex items-center justify-between">
                ...
                {isBasic ? (
                  <button
                    ...
                    onClick={() =>
                      alert(
                        "🔒 Feature Locked: Adding custom components & extra rows/columns is available only on Pro (₹599) and Yearly plans."
                      )
                    }
                  ...
                  >
                    <span>🔒</span>
                    <span>+ Add Component (Pro Only)</span>
                  </button>
                ) : (
                  <button
                    type="button"
            ...
            <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 space-y-4">
              ...
              {isBasic && historyList.length >= 3 && (
                <div className="p-3 rounded-lg bg-amber-950/40 border border-amber-500/30 text-[11px] text-amber-300">
                  ⚠️ 3/3 history limit reached on Basic. <Link href="/pricing" className="underline font-bold">Upgrade to Pro</Link> to save unlimited calculations.
                </div>
              )}
            </div>
